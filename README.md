...
## Clifford cutoff benchmarking suite

A standalone benchmarking harness for comparing Stim's Tableau simulator against
statevector baselines lives under `test_suite/`. It discovers the circuit depth
at which the Tableau + conversion path reaches a configurable speedup over a
statevector simulator while recording JSON metrics and optional plots. Refer to
[docs/test_suite_usage.md](docs/test_suite_usage.md) for installation and
