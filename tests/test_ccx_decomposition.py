...
def test_ccx_decomposition_unitary() -> None:
    """Decomposed CCX should match the native Toffoli unitary."""
    for n in range(3, 6):
        circ = Circuit([Gate("CCX", [0, 1, 2])], use_classical_simplification=False)
        assert all(g.gate.upper() != "CCX" for g in circ.gates)
        decomp = _matrix_from_gates(circ.gates, n)
        native_circuit = QuantumCircuit(n)
        native_circuit.ccx(0, 1, 2)
